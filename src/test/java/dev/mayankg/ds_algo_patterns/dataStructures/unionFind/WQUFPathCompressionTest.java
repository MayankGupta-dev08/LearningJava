...
class WQUFPathCompressionTest {

    private WQUFPathCompression unionFind;

    @BeforeEach
    void setUp() {
        unionFind = new WQUFPathCompression(10);
    }
    ...
    void testPathCompression() {
        // Create a scenario where path compression would significantly flatten the structure
        unionFind.union(0, 1);
        unionFind.union(1, 2);
        unionFind.union(2, 3);
        unionFind.union(3, 4);
        ...
        int rootBeforeCompression = unionFind.find(4);

        // Find operation on deepest element to trigger path compression
        int rootAfterCompression = unionFind.find(4);

        // Verify the root is the same before and after compression
        assertEquals(rootBeforeCompression, rootAfterCompression, "Root should remain the same before and after path compression.");

        // Verify the path compression by ensuring intermediate nodes point directly to the root
        assertEquals(rootAfterCompression, unionFind.find(3), "Path compression should make node 3 point directly to the root.");
        assertEquals(rootAfterCompression, unionFind.find(2), "Path compression should make node 2 point directly to the root.");
        assertEquals(rootAfterCompression, unionFind.find(1), "Path compression should make node 1 point directly to the root.");
        assertEquals(rootAfterCompression, unionFind.find(0), "Path compression should make node 0 point directly to the root.");
    }
}